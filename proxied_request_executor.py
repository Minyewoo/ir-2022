...
def parse_proxies_from_rows(rows):
    ...
    for row in rows:
        ...
        isnt_space_string = lambda x: x != ' '
        fields = list(filter(isnt_space_string, selector.css('td ::text').getall()))
...
class ProxiedRequestExecutor:
    ...
    def __init__(
        ...
        self.logger=logger
        self.logger.info('number of proxies found: %d', len(self.available_proxies))
    ...
    def unsafe_get(self, url: str,  params : dict = None):
        ...
        for _ in range(self.retry_count):
            ...
            if status not in (301, 302, 403, 404, 500, 503):
                ...
                self.logger.error('status: %s; banned: %s; url: %s', status, banned, url)
